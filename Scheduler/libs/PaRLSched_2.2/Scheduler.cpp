...
//	std::cout << "NUMA node of CPU 13 is  " << numa_node_of_cpu(13) << std::endl;
//	std::cout << "NUMA node of CPU 14 is " << numa_node_of_cpu(14) << std::endl;
//	std::cout << "NUMA node of CPU 15 is " << numa_node_of_cpu(15) << std::endl;
...
//		std::cout << "~~~~ thread " << it_map_actions->first << std::endl;
...
void Scheduler::optimize(const unsigned int& resource_ind)
{
	...
	for (it_map_actions; it_map_actions!=map_Actions_per_Thread_.end(); ++it_map_actions)
	{
		 ...
		 */
		if (sched_iteration_ % 10 == 0 && optimize_main_resource_ ) //|| it_map_estimates->second[resource_ind].random_switch_)
		{
			...
			for (unsigned int r = 0; r < RESOURCES_.size(); r++)
			{
				...
				if ((RESOURCES_[r].compare("NUMA_PROCESSING") == 0) && (CHILD_RESOURCES_[r].compare("CPU_PROCESSING") == 0))
				{
					...
					if (ST_mapping_)
					{
						...
						if (num_threads_per_resource[new_numa_node] >= ceil( 1 * num_threads_ / 2) && !found_most_popular_node)	// > ceil(2.7 * num_threads_ / 3)
						{
						...
						else if (num_threads_per_resource[new_numa_node] >= ceil( 1 * num_threads_ / 2) && found_most_popular_node)		// > ceil( 2.7 * num_threads_ / 3 )
						{
							current_popularity_ = num_threads_per_resource[new_numa_node];
						}
						else{
