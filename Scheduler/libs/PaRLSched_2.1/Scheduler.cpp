		...
		)
{
	...
	RL_mapping_ = RL_mapping;
	OS_mapping_ = OS_mapping;
	PR_mapping_ = PR_mapping;
	ST_mapping_ = ST_mapping;
	...
	LAMBDA_ 						= LAMBDA;
	gamma_ 							= gamma;
	RL_active_reshuffling_			= RL_active_reshuffling;
	RL_performance_reshuffling_ 	= RL_performance_reshuffling;
...
void Scheduler::estimate(const unsigned int& resource_ind)
{
	...
	for (it_map_actions; it_map_actions != map_Actions_per_Thread_.end(); ++it_map_actions)
	{
		...
		// double cur_balanced_performance = it_map_performances->second[resource_ind].overall_balanced_performance_;
...
void Scheduler::optimize(const unsigned int& resource_ind)
{
	...
	for (it_map_actions; it_map_actions!=map_Actions_per_Thread_.end(); ++it_map_actions)
	{
		 ...
		 */
		if (sched_iteration_ % 20 == 0 || it_map_estimates->second[resource_ind].random_switch_)
		{
			// std::cout << " !!!!!!!!!!!! Update main resource !!!!!!!!!!!!!!!!! \n";
		...
		for (unsigned int r = 0; r < RESOURCES.size(); r++)
		{
			...
			if (RESOURCES[r].compare("NUMA_PROCESSING")==0)
			{
				...
				if (CHILD_RESOURCES[r].compare("CPU_PROCESSING")==0)
					...
					estimate.initialize_w_child ( RESOURCES[r], max_num_numa_nodes_, MAX_NUMBER_MAIN_RESOURCES_[r], CHILD_RESOURCES[r], cpu_nodes_per_numa_node_, MAX_NUMBER_CHILD_RESOURCES_[r], i );
				else
					estimate.initialize(RESOURCES[r],max_num_numa_nodes_, MAX_NUMBER_MAIN_RESOURCES_[r], 0, true);
			}
...
//							std::cout << " current most popular node " << current_most_popular_node_ << " and previous one " << previous_most_popular_node_ << " and current popularity " <<
//									current_popularity_ << std::endl;
			...
			else if (OS_mapping_)
			{
				...
				for (unsigned int r = 0; r < RESOURCES_.size(); r++){
					...
					if ((RESOURCES_[r].compare("NUMA_PROCESSING") == 0) && (CHILD_RESOURCES_[r].compare("CPU_PROCESSING") == 0))
					{
						...
						// std::cout << " child resources : " << it_action->second[r].vec_child_sources_[0].size() << std::endl;
...
//					sleep(15);
					 ...
					 */
//					const int nodeMask = numa_node; // numa_node_of_cpu(cpu_node[0]);

//					const auto bindRc = mbind(stackAddr, stackSize, MPOL_BIND, &nodeMask, sizeof(nodeMask), MPOL_MF_MOVE); // MPOL_MF_MOVE | MPOL_MF_STRICT
//					assert(0 == bindRc);
...
//		}
	...
	if (RL_mapping_){
		...
		for (unsigned int cpu=0; cpu < 20; cpu++)
		{
