...
struct Struct_MethodsEstimate
{
			...
			const bool& RL_active_reshuffling, const bool& RL_performance_reshuffling, const bool& active_threads_change, const unsigned int & thread)
	{
		...
		if (RL_performance_reshuffling)
		{
			...
			if ((current_performance < 0.7 * current_run_ave_performance) && (!active_threads_change) && (max_estimate > 0.98))
			{
		...
		/* if (current_performance > maximum_performance && current_performance > 0)
		{
			maximum_performance = current_performance;
			step_size = 0.2 / maximum_performance;

		}*/
		step_size = 0.3 / current_run_ave_performance;
...
//			}
			...
			)
	{
		...
		if (current_run_ave_performance >= high_benchmark){
			...
			high_benchmark = 1.1 * current_run_ave_performance;
			low_benchmark = 0.7 * high_benchmark;
			random_switch = false;
			action_change = true;			// we allow the possibility to change actions, based on some randomization
		}
		else
		{
			...
			if (current_run_ave_performance <= low_benchmark)
			{
				...
				low_benchmark = 0.9 * current_run_ave_performance;
				high_benchmark = 1.3 * low_benchmark;
			}

		}


		/*std::cout << "     current running average performance = " << current_run_ave_performance <<
						"   previous run ave performance " << run_ave_performance_before << " and high benchmark " << high_benchmark << ", " << low_benchmark << std::endl;*/
				// sleep (1);
