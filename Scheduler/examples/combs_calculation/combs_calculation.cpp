...
#define ANT_COLONY_OPTIMIZATION 			0
#define COMBS_EXAMPLE						1

/* Parameters of Combinations Example */

#define THREAD_COUNT		  			20 // 20 fir heavy load
#define THREAD_GOAL					10

/*Scheduler Parameters*/
#define STEP_SIZE							0.005							// the standard one is 0.005
#define LAMBDA								0.03								// the standard one is 0.2
...
int main (int argc, char *argv[])
{
	...
	unsigned int num_cpus_1numa(1), num_cpus_2numa(1);

	if ( ( argc<6 || argc > 6) && ant_colony_optimization)
	{
	...
	else if (argc == 6 && ant_colony_optimization)
	{
	...
	else if (argc == 4 && combs_example)
	{
		num_workers = atoi(argv[1]);
		num_cpus_1numa = atoi(argv[2]);
		num_cpus_2numa = atoi(argv[3]);
	}
