...
{
  thread_info *targs = (thread_info *)args;

  for(int i = 0; i < targs->thread_goal; ++i) {
    AdvanceFrameMT(targs->thread_ref);
  }
...
void thread_create(thread_info* tinfo)
{
  ...
  for(int i = 0; i < threadnum; ++i) {
    tinfo[i].thread_ref = i;
    tinfo[i].thread_goal = framenum;
    tinfo[i].thread_num = i;

    std::cout << " thread " << i << " has reference " << tinfo[i].thread_ref << " and goal " << tinfo[i].thread_goal << std::endl;

    tinfo[i].status = 0;

    status = pthread_create(&tinfo[i].thread_id, &attr, thread_execute, &tinfo[i]);
